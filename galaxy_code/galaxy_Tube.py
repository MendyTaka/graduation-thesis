...
def Tube_L1(energy, N=100, eps=1e-6):
    ...
    if 'TubeL1_check.npy' in os.listdir(dirL1):
        ...
        if filename in TubeL1_check:
            ...
            U2_stable = np.load(dirL1+'U2_stable_'+filename+'.npy', allow_pickle=True).item()
            U3_stable = np.load(dirL1+'U3_stable_'+filename+'.npy', allow_pickle=True).item()
            U3_unstable = np.load(dirL1+'U3_unstable_'+filename+'.npy', allow_pickle=True).item()
            return {'U1_unstable':U1_unstable['U1_unstable'], 'U2_stable':U2_stable['U2_stable'],
                    'U3_stable':U3_stable['U3_stable'],'U3_unstable':U3_unstable['U3_unstable']}
    ...
    TubeL1['U2_stable'] = [0 for _ in range(N)]
    TubeL1['U3_stable'] = [0 for _ in range(N)]
    TubeL1['U3_unstable'] = [0 for _ in range(N)]
    ...
    for i in range(N):
        ...
        line3 = solve_ivp(func, [0,200], XUP,events=event_U3mp,rtol = 1.e-12, atol = 1.e-12)#U3
        line4 = solve_ivp(func, [0,200], XUM,events=event_U12pm,rtol = 1.e-12, atol = 1.e-12)
        line5 = solve_ivp(func, [0,-200], XSP,events=event_U3pm,rtol = 1.e-12, atol = 1.e-12)
        line6 = solve_ivp(func, [0,-200], XSM,events=event_U3pm,rtol = 1.e-12, atol = 1.e-12)#U3
        TubeL1['U1_unstable'][i] = line4.y
        TubeL1['U2_stable'][i] = line5.y
        TubeL1['U3_stable'][i] = line6.y
        TubeL1['U3_unstable'][i] = line3.y
        print(f'L1:{100*(i+1)//N}%')
    
    U1_unstable = {'U1_unstable':TubeL1['U1_unstable']}
    U2_stable = {'U2_stable':TubeL1['U2_stable']}
    U3_stable = {'U3_stable':TubeL1['U3_stable']}
    U3_unstable = {'U3_unstable':TubeL1['U3_unstable']}

    np.save(dirL1+'TubeL1_check.npy',np.append(TubeL1_check, filename))
    np.save(dirL1+'U1_unstable_{}.npy'.format(filename), U1_unstable)
    np.save(dirL1+'U2_stable_'+filename+'.npy', U2_stable)
    np.save(dirL1+'U3_stable_'+filename+'.npy', U3_stable)
    np.save(dirL1+'U3_unstable_'+filename+'.npy', U3_unstable)
...
def Tube_L2(energy, N=100, eps=1e-6):
    ...
    if 'TubeL2_check.npy' in os.listdir(dirL2):
        ...
        if filename in TubeL2_check:
            ...
            U1_stable = np.load(dirL2+'U1_stable_'+filename+'.npy', allow_pickle=True).item()
            U2_unstable = np.load(dirL2+'U2_unstable_'+filename+'.npy', allow_pickle=True).item()
            return {'U4_stable':U4_stable['U4_stable'],'U4_unstable':U4_unstable['U4_unstable'],
            'U2_unstable':U2_unstable['U2_unstable'],'U1_stable':U1_stable['U1_stable']}
    ...
    TubeL2['U2_unstable'] = [0 for _ in range(N)]
    TubeL2['U1_stable'] = [0 for _ in range(N)]
    ...
    for i in range(N):
        ...
        XSP = np.concatenate([x_init[0:4]+eps*YS, x_init[4:20]])
        XSM = np.concatenate([x_init[0:4]-eps*YS, x_init[4:20]])
        line3 = solve_ivp(func, [0,200], XUP, events=event_U12mp,rtol = 1.e-12, atol = 1.e-12)
        line4 = solve_ivp(func, [0,200], XUM, events=event_U4pm,rtol = 1.e-12, atol = 1.e-12)#U4
        line5 = solve_ivp(func, [0,-200], XSP, events=event_U4mp,rtol = 1.e-12, atol = 1.e-12)#U4
        line6 = solve_ivp(func, [0,-200], XSM, events=event_U4mp,rtol = 1.e-12, atol = 1.e-12)   
        TubeL2['U1_stable'][i] = line6.y
        TubeL2['U4_unstable'][i] = line4.y
        TubeL2['U2_unstable'][i] = line3.y
        TubeL2['U4_stable'][i] = line5.y
        print(f'L2:{100*(i+1)//N}%')
        
    U4_stable = {'U4_stable':TubeL2['U4_stable']}
    U4_unstable = {'U4_unstable':TubeL2['U4_unstable']}
    U2_unstable = {'U2_unstable':TubeL2['U2_unstable']}
    U1_stable = {'U1_stable':TubeL2['U1_stable']}
    ...
    np.save(dirL2+'U2_unstable_'+filename+'.npy', U2_unstable)
    np.save(dirL2+'U1_stable_'+filename+'.npy', U1_stable)
    return TubeL2
...
def main():
    ...
    ax = fig.add_subplot()
    #禁止領域
    z = hillregion.hillregion(E)
    ax.scatter(z[0],z[1],c='#a9a9a9',marker='s')
    #Tube
    for i in range(N):
        ax.plot(x1['U1_unstable'][i][0],x1['U1_unstable'][i][1],'r-')
        ax.plot(x1['U2_stable'][i][0],x1['U2_stable'][i][1],'b-')
        ax.plot(x1['U3_stable'][i][0],x1['U3_stable'][i][1],'b-')
        ax.plot(x1['U3_unstable'][i][0],x1['U3_unstable'][i][1],'r-')
        ax.plot(x2['U4_stable'][i][0],x2['U4_stable'][i][1],'b-')
        ax.plot(x2['U4_unstable'][i][0],x2['U4_unstable'][i][1],'r-')
        ax.plot(x2['U2_unstable'][i][0],x2['U2_unstable'][i][1],'r-')
        ax.plot(x2['U1_stable'][i][0],x2['U1_stable'][i][1],'b-')
    #Lyapunov軌道
    L1_con = Con_init(L[0][0], E)
    L2_con = Con_init(L[0][1], E)
    #積分計算
    y1 = Lyapunov(L1_con)
    y2 = Lyapunov(L2_con)
    ax.plot(y1[0], y1[1], 'g-')
